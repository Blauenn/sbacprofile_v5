	...
	return (
		<Tooltip title={title} placement="top" arrow>
			<div
				...
				className={`${approve_request_button_style} ${isSubmitting ? "bg-gray-500" : color
					}`}>
				<i
					className={`${approve_request_button_text_style} ${icon} ${isSubmitting ? "text-white" : textColor
						}`}></i>
			</div>
		</Tooltip>
	);
